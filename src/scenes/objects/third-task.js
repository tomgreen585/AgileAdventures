...
export default class ThirdTask extends TaskSkeleton {
    isCompleted = false;
    ...
    constructor(scene, x, y) {
        ...
        this.buttonLabels = ['Review', 'Plan', 'Test', 'Deploy', 'Launch', 'Develop', 'Design']  // Labels for the buttons;
        this.correctOrderButtonLabels = ["Plan", "Design", "Develop", "Test", "Deploy", "Review", "Launch"]  // Correct order of the buttons
        this.buttonMap = new Map();  // Create a map to store button-text pairs
        ...
        const backgroundOfTask = this.scene.add.image(33, 33, TASK_ASSET_KEYS.TASK_3)
            .setOrigin(0.0)
        ...
        this.taskHeight = 450 / 7;                        // total height is 450 (112.5 * 4)
        ...
        // Initialise the message text for incorrect order
        this.incorrectOrderText = this.scene.add.text(0, 0, '', {fontSize: '16px', fill: '#ff0000'})
            .setOrigin(0.5, 0.5)
            .setDepth(4); // Ensure it appears on top of other elements
        this.add(this.incorrectOrderText); // Add to the scene
        // Set the position of the incorrect order text box
        this.incorrectOrderText.x = (this.border.width / 3) * 2 + 50; // Position to the right of the main task area
        this.incorrectOrderText.y = this.yStart * 15; // Adjust y-position as needed
    }
    ...
    createDraggableButton(task) {
        ...
        this.add(button);
        this.add(buttonText); // Ensure the text is still part of the scene
        this.buttonMap.set(button, buttonText); // Store the button-text pair in the map to check for later
        return button;
    }
    ...
    checkIfDone(buttons) {
        ...
        // Check if the buttons in the first column are in the correct order
        // Sort buttons by row to ensure we're checking them in the right order
        const sortedButtons = this.buttons.slice().sort((a, b) => a.row - b.row);

        // Check if buttons are in the correct order
        for (let i = 0; i < sortedButtons.length; i++) {
            const button = sortedButtons[i];
            const buttonText = this.buttonMap.get(button).text; // Get the text from the Map
            if (buttonText !== this.correctOrderButtonLabels[i]) {
                console.log(`Incorrect order: ${buttonText} != ${this.correctOrderButtonLabels[i]}`);
                this.incorrectOrderText.setText(`Incorrect order: ${buttonText} != ${this.correctOrderButtonLabels[i]}`);
                return;
            }
        }
        // If all buttons are in the correct order, hide the incorrect order text
        this.incorrectOrderText.setText('');
        ...
        this.buttons.forEach(b => this.scene.input.setDraggable(b, false)); // Disable dragging of all
    ...
    drawTaskFrame() {
        ...
        // Draw lists text
        this.toDoText = this.writeListText("Project Steps", -1);
        this.writeListText("Ordered Steps", 1);
    ...
    createAgileExplanationBox() {
        ...
        const textBoxWidth = 375;
        const textBoxHeight = 440;

        const textBackground = this.scene.add.graphics();
        textBackground.fillStyle(7174787, 1);
        textBackground.fillRect((this.border.width / 3) * 2 - 80, this.yStart + 40, textBoxWidth, textBoxHeight);
        this.add(textBackground);

        const agileExplanationText = `
Agile Development Lifecycle:
- **Concept**: Define goals and requirements.
- **Inception**: Gather user stories and refine requirements.
- **Iteration**: Develop and test in short cycles (2-4 weeks).
- **Release**: Launch the product and gather feedback.
- **Maintenance**: Continuously improve based on user input.

Importance:
- **Flexibility**: Agile allows teams to adapt quickly to changes, ensuring that the final product meets user needs.
- **Customer Satisfaction**: Frequent releases and updates keep users engaged and happy.
- **Continuous Improvement**: Regular feedback helps teams to improve processes and product quality.
- **Collaboration**: Agile encourages teamwork, open communication, and shared responsibility among team members.
`;

        const agileText = this.scene.add.text(
            (this.border.width / 3) * 2 - 60, this.yStart + 40,
